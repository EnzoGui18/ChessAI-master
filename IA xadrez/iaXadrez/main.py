...
# Retorna um objeto movido com base na entrada do usuário. Não verifica se a movimentação é válida.
def get_user_move():
    ...
    try:
        ...
        yfrom = 8 - int(move_str[1:2]) # O tabuleiro é desenhado "de cabeça para baixo", então inverta a coordenada y.
        xto = letter_to_xpos(move_str[2:3])
        yto = 8 - int(move_str[3:4]) # O tabuleiro é desenhado "de cabeça para baixo", então inverta a coordenada y.
        return Move(xfrom, yfrom, xto, yto)
    except ValueError:
        print("Invalid format. Example: A2 A4")
        return get_user_move()

# Retorna uma movimentação válida com base na entrada do usuário.
...
# Converte uma letra (A-H) na posição x no tabuleiro de xadrez.
